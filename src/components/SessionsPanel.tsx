...
export default function SessionsPanel({
  ...
  const formatDateForSchedule = (createdAt: string | null) => {
    if (!createdAt) return 'Unknown date';
    const date = new Date(createdAt);
  ...
  // Group sessions by date for schedule view
  const groupedSessionsByDate = sessions.reduce((acc, session) => {
    if (!session.createdAt) return acc;
    const dateKey = formatDateForSchedule(session.createdAt);
    if (!acc[dateKey]) {
      acc[dateKey] = [];
    }
  ...
  const sortedDates = Object.keys(groupedSessionsByDate).sort((a, b) => {
    ...
    <div className={`h-full bg-white border-r border-gray-200 transition-all duration-300 ease-in-out overflow-hidden ${
        ...
        <div className="flex flex-col h-full w-80">
          ...
          <div className="bg-white border-b border-gray-200 px-4 py-4 flex-shrink-0">
            ...
            {/* Tabs */}
            <div className="flex items-center gap-1 border-b border-gray-200 -mx-4 px-4 mb-4">
              <button
